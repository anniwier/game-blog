...
r = r.replace("import ArticleEditorView", "import TagGamesView from \x27@/views/TagGamesView.vue\x27\nimport ArticleEditorView")
r = r.replace("\x27\x27,\n    }\n  ]", "\x27\x27,\n    },\n    {\n      path: \x27/tags/:tagId\x27,\n      name: \x27tagGames\x27,\n      component: TagGamesView,\n      meta: { title: \x27标签筛选\x27 }\n    }\n  ]")
open("game-blog-web/src/router/index.ts", "w", encoding="utf8").write(r)

# TagGamesView page
open("game-blog-web/src/views/TagGamesView.vue", "w", encoding="utf8").write("""<template>
  <div class="tag-page">
    <div class="tag-header"><h2>{{ tagName || \x27标签筛选\x27 }}</h2><span>共 {{ games.length }} 款游戏</span></div>
    <el-row :gutter="20">
      <el-col :xs="24" :sm="12" :md="8" :lg="6" v-for="g in games" :key="g.id">
        <el-card class="game-card" :body-style="{ padding: \x270px\x27 }" @click="$router.push(\x27/games/\x27 + g.id)">
